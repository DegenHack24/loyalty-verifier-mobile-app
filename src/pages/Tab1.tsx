...
const startScan = async (
  setDicountCardData: (data: DiscountCardData) => void
) => {
...
const Tab1: React.FC = () => {
  const [dicountCardData, setDicountCardData] =
    useState<DiscountCardData | null>(null);
    ...
    <IonPage>
      ...
      <IonContent fullscreen>
        <IonHeader collapse="condense">
          <IonToolbar>
            <IonButton
              onClick={() => startScan(setDicountCardData)}
              expand="block"
            >
              Scan
            </IonButton>
        ...
        {!!dicountCardData ? (
          <IonCard className="green-light">
            <img alt="" src={dicountCardData?.image} />
            <IonCardHeader>
              <IonCardTitle>{dicountCardData?.name}</IonCardTitle>
              <IonCardSubtitle className="text-color">
                Discount: {dicountCardData?.discount}
              </IonCardSubtitle>
              <IonCardSubtitle className="text-color">
                Shares: {dicountCardData?.shares}
              </IonCardSubtitle>
            </IonCardHeader>
            <IonCardContent className="text-color">
              {dicountCardData?.description}
            </IonCardContent>
          </IonCard>
        ) : null}
      </IonContent>
